...
test('$.datetime()', function () {
    equal($.datetime(), Math.floor(new Date().getTime()/1000), 'Current unix time');
    equal(typeof $.datetime(), 'number', 'Type is number');
});

test('$.datetime.isLeapYear()', function () {
    equal($.datetime.isLeapYear(), isLeapYear(yyyy), 'Current year is leap or not leap');
    equal($.datetime.isLeapYear(2013), false, '2013 is not leap year');
    equal($.datetime.isLeapYear(2012), true, '2012 is leap year');
    equal($.datetime.isLeapYear(2000), true, '2000 is leap year');
    equal($.datetime.isLeapYear(1900), false, '1900 is not leap year');
    equal($.datetime.isLeapYear(1904), true, '1904 is leap year');
    equal($.datetime.isLeapYear(1905), false, '1905 is not leap year');
});
